...
// platform.
//
WorkQueue::WorkQueue(std::deque<std::function<void()>> &worklist, int numWorkers) :
    m_work(worklist)
{
    if (numWorkers < 1) {
        numWorkers = std::thread::hardware_concurrency();
...
void
WorkQueue::submit(std::function<void()> job, bool blocking)
{
