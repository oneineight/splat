 ...
 ** ftp://flattop.its.bldrdoc.gov/itm/ITMDLL.cpp prior to May, 2007. C++
 ...
 ** by the NTIA/ITS on June 26, 2007. With correction set SS1 and SS2:
 *itm71.cpp.    *
 ...
 ** range. * Update 8 Jun 2010 to modify alos to match 2010 series of IEEE-BTS *
 * newsletter articles * Update June 12, 2010 to z version to change test
 *outputs                      * Offshoot start date June 23, 2010 to start
 *itwom2.0 dual version for FCC.     * Update to 2.0b July 25 to correct if
 *statement errors in adiff2 re two peak   * calculations starting at line 525 *
 ...
 ** Modified to make 1st obs loss=5.8 only, no clutter loss considered * Updated
 *to 3.1 after much cleanup, conversion to pure C, and added           *
...
double tcreal(tcomplex tc) { return tc._real; }
...
double tcimag(tcomplex tc) { return tc._imag; }

double tcarg(tcomplex tc) { return atan2(tc._imag, tc._real); }
...
tcomplex tcdiv(tcomplex a, tcomplex b) {
    tcomplex ret = {(a._real * b._real + a._imag * b._imag) /
                    ...
                    (a._imag * b._real - a._real * b._imag) /
                        (b._real * b._real + b._imag * b._imag)};
    return ret;
}

tcomplex tcsqrt(tcomplex tc) {
    tcomplex ret = {sqrt(tcabs(tc)) * cos(tcarg(tc) / 2),
                    sqrt(tcabs(tc)) * sin(tcarg(tc) / 2)};
    return ret;
}
 ...
 *
 * v2: the square of the value v, where v is the "internal wedge phase angle" of
 * an obstruction
 *
 * Returns the attenuation factor
 *
 * See ITWOM-SUB-ROUTINES.pdf, p10-11 for the definition of v2, and p405 for the
    ...
    if (dist == 0.0) {
        ...
        if (prop->tgh > prop->cch) /* for TX ant above all clutter height*/
        {
            ...
            for (int j = 0; j < 5; ++j) {
                ...
                ucrpc = sqrt((hone - prop->cch + hc) * (hone - prop->cch + hc) +
                             (dx * dx));
            ...
            if (d1a < 50.0) {
                ...
                if (d1a < 225.0) {
                    ...
                    arte = q +
                           (0.7 * pdk - max(0.01, log10(prop->wn * 47.7) - 2)) *
                               (prop->hg[1] / hone);
                }

                else {
                    q = 0.00055 * (pdk) +
                        log10(pdk) * (0.041 - 0.0017 * sqrt(hone) + 0.019);
                    ...
                    if (pdk > zi) {
                        q = (pdk - zi) * 10.2 *
                            ((sqrt(max(0.01, log10(prop->wn * 47.7) - 2.0))) /
                             (100 - zi));
                    } else {
                        q = ((zi - pdk) / zi) *
                            (-20.0 * max(0.01, log10(prop->wn * 47.7) - 2.0)) /
                            sqrt(hone);
                    }
                    arte = arte + q;
                }
        ...
        {
            q = (prop->cch - prop->tgh) *
                (2.06943 - 1.56184 * exp(1 / prop->cch - prop->tgh));
            q = q + (17.98 - 0.84224 * (prop->cch - prop->tgh)) *
                        exp(-0.00000061 * pd);
            arte = q + 1.34795 * 20 * log10(pd + 1.0);
            arte = arte - (max(0.01, log10(prop->wn * 47.7) - 2)) *
                              (prop->hg[1] / prop->tgh);
        }
        saalosv = arte;
    }
...
typedef struct adiff_state {
    ...
    double afo; /* attenuation from absorption/scattering from oxygen, water
                   vapor, and terrain clutter */
    double qk;
    double aht;
    double xht;
} adiff_state;
...
void adiff_init(adiff_state *state, prop_type *prop, propa_type *propa) {
    ...
    state->afo = min(15.0, 2.171 * log(1.0 + 4.77e-4 * prop->hg[0] *
                                                 prop->hg[1] * prop->wn * q));
 ...
 * See ITWOM-SUB-ROUTINES.pdf, p2
 */
double adiff(adiff_state *state, double dist, prop_type *prop,
             propa_type *propa) {
    ...
    q = (state->wd1 + state->xd1 / dist) *
        min(((1.0 - 0.8 * exp(-dist / 50e3)) * prop->dh * prop->wn), 6283.2);
    wd = 25.1 / (25.1 + sqrt(q));
...
void adiff2_init(adiff2_state *state, prop_type *prop, propa_type *propa) {
    ...
    /* coefficients for a standard four radii, rounded earth computation are
     * prepared */
    state->wd1 = sqrt(1.0 + state->qk / q);
    state->xd1 = propa->dla + propa->tha / prop->gme;
    q = (1.0 - 0.8 * exp(-propa->dlsa / 50e3)) * prop->dh;
 ...
 * See ITWOM-SUB-ROUTINES.pdf, p10
 */
double adiff2(adiff2_state *state, double dist, prop_type *prop,
              propa_type *propa) {
    ...
    double kedr = 0.0, arp = 0.0, sdr = 0.0, pd = 0.0, srp = 0.0, kem = 0.0,
           csd = 0.0, sdl = 0.0, adiffv2 = 0.0, closs = 0.0;

    /* sf1=1.0; */ /* average empirical hilltop foliage scatter factor for 1
                      obstruction  */
    sf2 = 1.0;     /* average empirical hilltop foliage scatter factor for 2
                      obstructions */
    ...
    toh = prop->hht -
          (prop->rch[0] -
           prop->dl[0] * ((prop->rch[1] - prop->rch[0]) / prop->dist));
    roh = prop->hhr -
          (prop->rch[0] - (prop->dist - prop->dl[1]) *
                              ((prop->rch[1] - prop->rch[0]) / prop->dist));
    toho = prop->hht -
           (prop->rch[0] - (prop->dl[0] + dsl) * ((prop->hhr - prop->rch[0]) /
                                                  (prop->dist - prop->dl[1])));
    ...
    dtr = sqrt(prop->dist * prop->dist +
               (prop->rch[0] - prop->rch[1]) * (prop->rch[0] - prop->rch[1]));
    dtr += prop->gme * prop->dist;
    dhh1 = sqrt((prop->dist - propa->dla) * (prop->dist - propa->dla) +
                toho * toho);
    dhh1 += prop->gme * (prop->dist - propa->dla);
    dhh2 = sqrt((prop->dist - propa->dla) * (prop->dist - propa->dla) +
                roho * roho);
    dhh2 += prop->gme * (prop->dist - propa->dla);

    /* for 1 obst tree base path */
    dtof =
        sqrt(prop->dl[0] * prop->dl[0] + (toh - prop->cch) * (toh - prop->cch));
    dtof += prop->gme * prop->dl[0];
    dto1f = sqrt(prop->dl[0] * prop->dl[0] +
                 (toho - prop->cch) * (toho - prop->cch));
    dto1f += prop->gme * prop->dl[0];
    drof =
        sqrt(prop->dl[1] * prop->dl[1] + (roh - prop->cch) * (roh - prop->cch));
    drof += prop->gme * (prop->dl[1]);
    dro2f = sqrt(prop->dl[1] * prop->dl[1] +
    ...
    } else /* for single obstacle */
    {
        ...
        if (trunc(prop->dl[1]) > 0.0) /* receive site past 1st peak */
        {
            ...
            if (prop->the[1] <
                0.2) /* receive grazing angle less than .2 radians */
            {
                vv = 0.6365 * prop->wn * fabs(dto + dro - dtr);
                ...
                if (prop->hht < 3400) {
                    ...
                    /* report pd prior to restriction
                       keep pd between 0 and pi radians and adjust for 3&4
                       quadrant */
                    if (pd >= 3.141592654) {
                        pd = 6.283185307 - pd;
 ...
 */
double ascat(ascat_state *state, double dist, prop_type *prop,
             propa_type *propa) {
    ...
    if (state->h0s > 15.0)
        ...
        if (et < 1.0) {
            ...
            h0 = et * h0 +
                 (1.0 - et) * 4.343 *
                     log((temp * temp) * (r1 + r2) / (r1 + r2 + 2.8284));
        }
    ...
    ascatv = ahd(th * dist) +
             ...
             0.1 * (prop->ens - 301.0) * exp(-th * dist / 40e3) + h0;
 ...
 *
 * IN:
 * fmhz: frequency at which to do the study, in mhz. Passed in to
 * point_to_point() or area() zsys: average elevation of the study, usually
 * calculated by averaging the elevation array en0: atmospheric refractivity at
 * surface level, passed in to point_to_point() ipol: polarity (0 or 1). passed
 * in to point_to_point() eps: polarization constant. passed in to
 * point_to_point() sgm: ground constant. passed in to point_to_point()
 *
 * IN/OUT:
 * prop: filled in by various calculations on these values.
 *
 * Note that nothing here is particularly compute-intensive to calculate.
 * However the values usually stay the same over a study. A (minor) performance
 * enhancement would be to calculate them once and save them for all studies of
 * that tx point.
 *
 ...
 */
double alos(alos_state *state, double dist, prop_type *prop,
            propa_type *propa) {
    tcomplex prop_zgnd = {prop->zgndreal, prop->zgndimag};
    tcomplex r = {0.0, 0.0};
    double s, sps, q;
...
double alos2(prop_type *prop, propa_type *propa) {
    ...
    if (prop->mdp < 0) {
        ...
        if (dr < (0.5 * pd)) {
            drh = 6378137.0 -
                  sqrt(-(0.5 * pd) * (0.5 * pd) + 6378137.0 * 6378137.0 +
                       (0.5 * pd - dr) * (0.5 * pd - dr));
        } else {
            drh = 6378137.0 -
                  sqrt(-(0.5 * pd) * (0.5 * pd) + 6378137.0 * 6378137.0 +
        ...
        if ((sps < 0.05) && (prop->cch > hrg) &&
            (prop->dist < prop->dl[0])) /* if far from transmitter and receiver
                                           below canopy */
        {
            cd = max(0.01, pd * (prop->cch - hrg) / (htg - hrg));
            cr = max(0.01, pd - dr + dr * (prop->cch - drh) / htg);
            q = ((1.0 - 0.8 * exp(-pd / 50e3)) * prop->dh *
                 (min(-20 * log10(cd / cr), 1.0)));
        }
    }
    ...
    for (int j = 0; j < 2; ++j) {
        if (kst[j] <= 0)
            ...
            prop->he[j] =
                prop->hg[j] +
                (1.0 + q) *
                    exp(-min(20.0, 2.0 * prop->hg[j] / max(1e-3, prop->dh)));
        }
 ...
 *
 * This was replaced by lrprop2 because no provision was made to accommodate the
 * changes required in the computational procedure that must occur when the
 * line-of-sight and diffraction ranges no longer gently merge, but are,
 * instead, abruptly separated by a major obstruction.
 *
...
void lrprop(double dist, prop_type *prop, propa_type *propa) {
    ...
    if (prop->mdp != 0) {
        ...
        /* Warning: tx take-off angle, antenna horizon < 1/10 of horizon
         * distance */
        for (j = 0; j < 2; j++)
            if (fabs(prop->the[j]) > 200e-3 ||
                prop->dl[j] < 0.1 * propa->dls[j] ||
                prop->dl[j] > 3.0 * propa->dls[j])
                prop->kwx = max(prop->kwx, 3);

        /* Out-of-Range: refractivity, earth curvature, earth
         * impedance/reactance, wave number */
        if (prop->ens < 250.0 || prop->ens > 400.0 || prop->gme < 75e-9 ||
            prop->gme > 250e-9 ||
            (tcreal(prop_zgnd) <= fabs(tcimag(prop_zgnd))) ||
        ...
        /* xae=pow(prop->wn*pow(prop->gme,2.),-THIRD); -- JDM made argument 2 a
         * double */
        prop->xae =
    ...
    /* if the propagation model is in "initializing area mode", reset it to
     * normal running */
    if (prop->mdp >= 0) {
        prop->mdp = 0;
        prop->dist = dist;
    ...
    if (prop->dist > 0.0) {
        ...
        double dmin =
            fabs(prop->he[0] - prop->he[1]) /
            200e-3; /* minimum acceptable path distance length (meters) */
        if (prop->dist < dmin)
            prop->kwx = max(prop->kwx, 3);
    ...
    if (prop->dist < propa->dlsa) {
        if (!prop->wlos) {
            ...
            if (d0 < d1) {
                ...
                q = log(d2 / d0);
                propa->ak2 =
                    max(0.0, ((d2 - d0) * (a1 - a0) - (d1 - d0) * (a2 - a0)) /
    ...
    if (prop->dist <= 0.0 || prop->dist >= propa->dlsa) {
        if (!prop->wscat) {
            ...
            if (a5 < 1000.0) {
                ...
                propa->dx =
                    max(propa->dlsa,
                        max(propa->dla + 0.3 * prop->xae * log(47.7 * prop->wn),
...
void lrprop2(double dist, prop_type *prop, propa_type *propa) {
    ...
    if (prop->dist > 0.0) {
        if (prop->dist > 1000e3) /* prop->dist being in meters, if greater than
                                    1000 km, kwx=1 */
            prop->kwx = max(prop->kwx, 1);
    ...
    if (prop->dist < propa->dlsa) {
        ...
        } else /* for ITWOM point-to-point mode */
        {
            ...
            if (prop->los == true) /* if line of sight */
            {
                ...
                } else if (trunc(prop->dist - prop->dl[0]) >
                           0.0) /* if past 1st horiz */
                {
                    adiff2_init(&state, prop, propa);
 ...
 *
 * This subroutine calculates the additional loss resulting from statistical
 * analysis of long term variability due to time (reliability), location, and/or
 * situational variability (confidence), depending upon the operating mode
 * determined by the mode of variability code input. The subroutine reports out
 * a single value of loss, avarv, in decibels (dB), which includes aref, the
 * "reference attenuation" summed with the additional statistical loss.
 *
...
const double xv1[7] = {144.9e3, 228.9e3, 262.6e3, 84.1e3,
                       228.9e3, 141.7e3, 2222.e3};
const double xv2[7] = {190.3e3, 205.2e3, 185.2e3, 101.1e3,
                       205.2e3, 315.9e3, 164.8e3};
const double xv3[7] = {133.8e3, 143.6e3, 99.8e3, 98.6e3,
...
const double xsm1[7] = {762.2e3, 100.4e3, 138.2e3, 139.1e3,
                        93.7e3,  187.8e3, 609.8e3};
const double xsm2[7] = {123.6e3, 172.5e3, 242.2e3, 132.7e3,
                        186.8e3, 169.6e3, 119.9e3};
const double xsm3[7] = {94.5e3,  136.4e3, 178.6e3, 193.5e3,
...
const double xsp1[7] = {636.9e3, 138.7e3, 165.3e3, 464.4e3,
                        93.2e3,  216.0e3, 136.2e3};
const double xsp2[7] = {134.8e3, 143.7e3, 225.7e3, 93.1e3,
                        135.9e3, 152.0e3, 188.5e3};
const double xsp3[7] = {95.6e3,  98.6e3,  129.7e3, 94.2e3,
...
double avar(double zzt, double zzl, double zzc, prop_type *prop,
    ...
    int kdv;
    bool ws, w1;
    double dexa, de, vmd, vs0, sgl, sgtm, sgtp, sgtd, tgtd, gm, gp, cv1, cv2,
        yv1, yv2, yv3, csm1, csm2, ysm1, ysm2, ysm3, csp1, csp2, ysp1, ysp2,
        ysp3, csd1, zd, cfm1, cfm2, cfm3, cfp1, cfp2, cfp3;
    ...
    vs = vs0 + (temp1 * temp1) / (rt + zc * zc) +
         (temp2 * temp2) / (rl + zc * zc);
 ...
 *
 * This version is only used only with ITM 1.2.2. An updated version with
 * improved obstacle handling and that does caching for later calculations is
 * below (hzns2).
 *
...
void hzns(const elev_t pfl[], prop_type *prop) {
    ...
    za = pfl[2] +
         prop->hg[0]; /* transmitter location height plus ant height AGL      */
    zb = pfl[np + 2] +
         prop->hg[1]; /* receiver location height plus ant height AGL         */

    qc = 0.5 * prop->gme; /* half the earth's curvature, or 1/(2r) */
    q = qc * prop->dist;  /* q is now dist/2r (i.e. radians)  */

    prop->the[1] =
        (zb - za) / prop->dist; /* temporarily set slope from tx to rx. */
    /* XXX BUG. This really should be an arctan. See        */
    /* ITWOM pp156-157.                                     */

    prop->the[0] = prop->the[1] - q; /* set takeoff angle of receiver */
    prop->the[1] = -prop->the[1] -
                   q; /* and set takeoff angle of transmitter to the opposite */

    prop->dl[0] =
        prop->dist; /* optimistically set tx horizon to longest path        */
    ...
    for (int i = 1; i < np; i++) {
        ...
        q = pfl[i + 2] - ((qc * sa + prop->the[0]) * sa) -
            za; /* height difference from pfl pt to sa, */
 ...
 *
 *
 * This version has improved obstacle handling and does some caching for later
 * calculations.
 *
 * See ITWOM-SUB-ROUTINES.pdf, page 150
 */
void hzns2(const elev_t pfl[], prop_type *prop) {
    ...
    prop->the[1] = atan(
        (zb - za) / prop->dist); /* temporarily set the slope from tx to rx */
    ...
    if (np >= 2) {
        ...
        for (j = 1; j < np; j++) {
            ...
            q = pfl[j + 2] - ((qc * sa + prop->the[0]) * sa) -
                za; /* height diff from pfl pt to sa,    */
            ...
            if (q > 0.0) /* tx obstruction found?                */
            {
                ...
                prop->the[0] =
                    min(prop->the[0],
                        1.569); /* limit the max radians of takeoff angle */
                prop->hht = pfl[j + 2]; /* Note: this can be 0, causing dr
                                           (below) to blow up! */
                wq = false;
            }
        }
        ...
        if (!wq) /* we know it's not LOS so examine the rx end   */
        {
            for (i = 1; i < np; i++) {
                ...
                if (q > 0.0) {
                    ...
                    prop->the[1] =
                        min(prop->the[1],
                            1.57); /* limit the rx take-off angle range */
                    prop->the[1] = max(prop->the[1], -1.568);

                    prop->hhr = pfl[np + 2 - i]; /* NOTE: this can be 0, causing
                                                    dr (below) to blow up! */
            ...
            prop->the[0] = atan((prop->hht - za) / prop->dl[0]) -
                           0.5 * prop->gme * prop->dl[0];
            prop->the[1] = atan((prop->hhr - zb) / prop->dl[1]) -
     ...
     *
     * dr is the distance in meters from the last obstruction peak to the
     * reflection point.
     */
    dr = 0.0;
    if ((prop->dl[1]) < (prop->dist)) {
        dshh = prop->dist - prop->dl[0] -
               prop->dl[1]; /* distance between obstacle peaks */
        ...
        {
            if (prop->hhr > 0.0) { /* if the antenna has 0 height, there is no
                                      2-ray reflection */
 ...
 * Used only with ITM 1.2.2
 */
void z1sq1(const elev_t z[], const double x1, const double x2, double *z0,
           double *zn) {
    ...
    xb = xn -
         trunc(FORTRAN_DIM(xn, x2 / z[1])); /* set our end interval count  */

    if (xb <=
        xa) /* if we have a very short path, xb can be less than xa, so... */
    {
    ...
    a = 0.5 * (z[ja + 2] + z[jb + 2]); /* avg of the start and end heights, i.e.
                                          initial intercept, a */
    b = 0.5 * (z[ja + 2] - z[jb + 2]) *
        x; /* avg of the start/end height difference times the run, i.e initial
              slope */
 ...
 */
void z1sq2(const elev_t z[], const double x1, const double x2, double *z0,
           double *zn) {
 ...
 * before ir are greater than or equal to all the elements after ir. In
 * particular, a(ir) will have the same value it would have if a were completely
...
double d1thx(const elev_t pfl[], const double x1, const double x2) {
    ...
    for (j = 0; j < n; j++) {
        s[j + 2] -=
            xa; /* difference between the original height and new slope */
        xa = xa + xb;
    }
 ...
 */
void qlrpfl(const elev_t pfl[], int klimx, int mdvarx, prop_type *prop,
    ...
    for (j = 0; j < 2; j++) /* for both tx and rx... */
        xl[j] = min(15.0 * prop->hg[j],
                    0.1 * prop->dl[j]); /* ...set xl to min of 15x ant height or
                                           1/10 horizon dist */

    xl[1] =
        prop->dist - xl[1]; /* adjust the rx distance to be from the far end */

    prop->dh = d1thx(pfl, xl[0],
                     xl[1]); /* calculate the terrain irregularity factor */
    ...
    if (prop->dl[0] + prop->dl[1] > 1.5 * prop->dist) {
        ...
        /* set effective heights to endpoint heights plus an offset. See ITWOM
         * p236 for discussion */
        prop->he[0] = prop->hg[0] + FORTRAN_DIM(pfl[2], za);
        prop->he[1] = prop->hg[1] + FORTRAN_DIM(pfl[np + 2], zb);

        /* arcana to (re)determine dl values that we initially got from hzns.
         * See ITWOM. */
        for (j = 0; j < 2; j++)
        ...
        if (q <= prop->dist) /* if there is a rounded horizon, or two
                                obstructions, in the path */
        {
            ...
            for (j = 0; j < 2; j++) {
                prop->he[j] *= q; /* tx effective height set to be path
        ...
        for (j = 0; j < 2; j++) /* original empirical adjustment?  uses delta-h
                                   to adjust grazing angles */
        {
            q = sqrt(2.0 * prop->he[j] / prop->gme);
 ...
 */
void qlrpfl2(const elev_t pfl[], int klimx, int mdvarx, prop_type *prop,
             propa_type *propa, propv_type *propv) {
    int np, j;
    ...
    xl[1] =
        prop->dist - xl[1]; /* adjust the rx distance to be from the far end */
    prop->dh = d1thx2(pfl, xl[0],
                      xl[1]); /* calculate the terrain irregularity factor */

    /* The first branch of this if statement is for when there are no points in
     * array (e.g. called in the now-deprecated "area" mode, or when the
     * distance between points is very large.
     *
     * In other words, it's not really ever used in modern code.
     */
    if ((np < 1) || (pfl[1] > 150.0)) {
        /* TRANSHORIZON; diffraction over a mutual horizon, or for one or more
         * obstructions */
        if (dlb < 1.5 * prop->dist) {
            z1sq2(pfl, xl[0], 0.9 * prop->dl[0], &za,
                  &q); /* fit line to terrain from start to 90% of start
                          horizon/obstacle */
            z1sq2(pfl, prop->dist - 0.9 * prop->dl[1], xl[1], &q,
                  &zb); /* ditto, but from the other end */

            prop->he[0] =
                prop->hg[0] +
                FORTRAN_DIM(pfl[2],
                            za); /* set effective height at start (ASL) */
            prop->he[1] =
                prop->hg[1] +
                FORTRAN_DIM(pfl[np + 2], zb); /* ...and at end (ASL) */
        }
        ...
        /* Line-of-Sight path */
            ...
            prop->he[1] =
                prop->hg[1] +
                FORTRAN_DIM(pfl[np + 2],
                            zb); /* set effective height at end (ASL) */

            /* this is essentially calculating the horizon distances, using the
             * earth's curvature as an approximation. The calculation can be
             * done much more accurately using the hzns call
             ...
             * This was inoperative in the ITM FORTRAN and DLL. It was
             * reactivated and tested and found to be unstable. It was therefore
             * reburied (made nonfunctional). See the discussion in ITWOM p250.
             */
            ...
            /* re-recalculate the horizon distances, using the earth's curvature
             * as an approximation. */
            /* Obsolete. */
            for (j = 0; j < 2; j++) {
                prop->he[j] *= q;
            ...
            /* this sets (or resets) prop->the, and is not in The Guide FORTRAN
             * QLRPFL */
            for (j = 0; j < 2; j++) {
                q = sqrt(2.0 * prop->he[j] / prop->gme);
                prop->the[j] = (0.65 * prop->dh * (q / prop->dl[j] - 1.0) -
    ...
    } else /* for ITWOM ,computes he for tx, rcvr, and the receiver approach
              angles for use in saalos */
    {

        prop->he[0] = prop->hg[0] +
                      (pfl[2]); /* set the effective height at start (ASL)  */
        prop->he[1] =
            prop->hg[1] +
            (pfl[np + 2]); /* set the effective height at end (ASL)    */

        rad = (prop->dist - 500.0); /* receiver approach path start location. We
                                       consider the last half-km only */

        if (prop->dist > 550.0) {
            z1sq2(pfl, rad, prop->dist, &rae1,
                  &rae2); /* do a least-squares fit on that last half-km.*/
        } else {
            rae1 = 0.0;
            rae2 = 0.0;
        }

        /* theta, receive angle, is the slope of the least-squares line. This
         * could be returned by z1sq2, but recovering it isn't that hard. It's
         * only used in alos(). */
        ...
        /* theta, near receiver, is the slope of the terrain right next to the
         * receiver. It's only used in alos(). */
...

{
    ...
    qlrps(frq_mhz, zsys, q, pol, eps_dielect, sgm_conductivity,
          &prop); /* quick longley rice - setup */

    qlrpfl(elev, propv.klim, propv.mdvar, &prop, &propa,
           &propv); /* quick longley-rice, do the calculation */
...
enc_ncc_clcref         clutter refractivity; 1000 N-units to match ITU-R
P.1546-2
...
[height(meters) point n] propmode:  Value   Mode -1     mode is undefined 0 Line
of Sight 5     Single Horizon, Diffraction 6     Single Horizon, Troposcatter 9
Double Horizon, Diffraction 10     Double Horizon, Troposcatter errnum: 0- No
Error. 1- Warning: Some parameters are nearly out of range. Results should be
used with caution. 2- Note: Default parameters have been substituted for
...
void point_to_pointMDH_two(const elev_t elev[], double tht_m, double rht_m,
                           ...
                           double delta_h_diff, double frq_mhz,
                           int radio_climate, int pol, int mode_var,
...

{
    ...
    *dbloss = avar(ztime, zloc, zconf, &prop, &propv) +
              fs; /*avar(time,location,confidence) */
    *errnum = prop.kwx;
}
...
conf, rel: .01 to .99
elev[]: [num points - 1], [delta dist(meters)], [height(meters) point 1], ...,
[height(meters) point n] errnum: 0- No Error. 1- Warning: Some parameters are
nearly out of range. Results should be used with caution. 2- Note: Default
parameters have been substituted for impossible ones. 3- Warning: A combination
of parameters is out of range. Results are probably invalid. Other-  Warning:
...
void point_to_pointDH(const elev_t elev[], double tht_m, double rht_m,
                      ...
                      int pol, double conf, double rel, double loc,
    ...
    *dbloss = avar(zr, 0.0, zc, &prop, &propv) +
              fs; /*avar(time,location,confidence) */
    *errnum = prop.kwx;
}
...
4-Desert, 5-Continental Temperate, 6-Maritime Temperate, Over Land,
7-Maritime Temperate, Over Sea
ModVar: 0 - Single: pctConf is "Time/Situation/Location", pctTime, pctLoc not
used 1 - Individual: pctTime is "Situation/Location", pctConf is "Confidence",
pctLoc not used 2 - Mobile: pctTime is "Time/Locations (Reliability)", pctConf
is "Confidence", pctLoc not used 3 - Broadcast: pctTime is "Time", pctLoc is
"Location", pctConf is "Confidence" pctTime, pctLoc, pctConf: .01 to .99 errnum:
0- No Error. 1- Warning: Some parameters are nearly out of range. Results should
be used with caution. 2- Note: Default parameters have been substituted for
impossible ones. 3- Warning: A combination of parameters is out of range.
Results are probably invalid.
Other-  Warning: Some parameters are out of range.
Results are probably invalid.
...
double ITMAreadBLoss(long ModVar, double deltaH, double tht_m, double rht_m,
    ...
    area(ModVar, deltaH, tht_m, rht_m, dist_km, TSiteCriteria, RSiteCriteria,
         ...
         clutter_height, clutter_density, delta_h_diff, frq_mhz, radio_climate,
         pol, mode_var, pctTime, pctLoc, pctConf, &dbloss, strmode, &errnum);
    return dbloss;
}

double ITWOMVersion() { return 3.0; }
