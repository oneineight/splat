 ...
 * Globals
 *****************************/
typedef enum AppMode { APPMODE_NORMAL = 0, APPMODE_HD = 1 } AppMode;
...
char *dec2dms(double decimal) {
    ...
    buf[0] = 0;
    snprintf(buf, MAX_LINE_LEN, "%d%c %d\' %d\"", degrees * sign, 176, minutes,
             seconds);
    return (buf);
}
...
DEM *AllocateDEM() {
    ...
    do {
        ...
        dem->mask =
            (unsigned char **)malloc(dem->arysize * sizeof(unsigned char *));
        dem->signal =
            (unsigned char **)malloc(dem->arysize * sizeof(unsigned char *));
...
unsigned long SizeofDEM(AppMode mode) {
    ...
    return sizeof(DEM) + (sizeof(short *) * arysize) +
 ...
 */
DEM *FindDEM_Explicit(double minlat, double minlon, double maxlat,
                      double maxlon) {
    for (int i = 0; i < demCount; ++i) {
        if (minlat == aDEM[i]->min_north && minlon == aDEM[i]->min_west &&
            maxlat == aDEM[i]->max_north && maxlon == aDEM[i]->max_west)
            return aDEM[i];
    }
    return NULL;
}
 ...
 * longitude given.
 */
int GetMask(double lat, double lon) { return (OrMask(lat, lon, 0)); }
...
double Azimuth(Site source, Site destination) {
    ...
    angle = sin(src_lat) * sin(dest_lat) +
...
double ElevationAngle(Site source, Site destination) {
    ...
    return ((180.0 * (acos(((b * b) + (dx * dx) - (a * a)) / (2.0 * b * dx))) /
             PI) -
            90.0);
}
...
void ReadPath(Site &source, Site &destination, Path *path) {
    ...
    double azimuth, distance, lat1, lon1, beta, den, num, lat2, lon2,
        total_distance, dx, dy, path_length, miles_per_sample,
...
double ElevationAngle2(Site source, Site destination, double er) {
    int x;
    char block = 0;
    double source_alt, destination_alt, cos_xmtr_angle, cos_test_angle,
        test_alt, elevation, distance, source_alt2,
    ...
    cos_xmtr_angle = ((source_alt2) + (distance * distance) -
    ...
    for (x = 2, block = 0; x < path->length && block == 0; x++) {
        ...
        test_alt = earthradius + (path->elevation[x] == 0.0
                                      ? path->elevation[x]
 ...
 * returned.
 */
double AverageTerrain(Site source, double azimuthx, double start_distance,
                      double end_distance) {
    ...
    if (GetElevation(destination) < -4999.0)
        ...
        for (c = 0, samples = 0; c < endpoint; c++) {
            if (path->distance[c] >= start_distance) {
                terrain +=
                    (path->elevation[c] == 0.0 ? path->elevation[c]
                                               : path->elevation[c] + clutter);
                samples++;
            }
        }
...
void PlaceMarker(Site location) {
    ...
    if (lat < xmax && lat >= xmin && (LonDiff(lon, ymax) <= 0.0) &&
        (LonDiff(lon, ymin) >= dpp)) {
        p1 = 1.0 / ppd;
        p3 = 3.0 / ppd;
        p6 = 6.0 / ppd;
        p8 = 8.0 / ppd;
        ...
        for (a = 0, x = lat - p3; (x <= xmax && x >= xmin && a < 7);
             x += p1, a++)
            for (b = 0, y = lon - p3; (LonDiff(y, ymax) <= 0.0) &&
                                      (LonDiff(y, ymin) >= dpp) && b < 7;
                 y += p1, b++)
        ...
        if (occupied == 0) {
            ...
            if ((LonDiff(lon + label_length, ymax) <= 0.0) &&
                (LonDiff(lon - label_length, ymin) >= dpp)) {
...
Site LoadQTH(char *filename) {
    ...
    if (fd != NULL) {
        ...
        for (x = 0; buf[x] != 13 && buf[x] != 10 && buf[x] != 0 &&
                    x < (MAX_SITE_NAME_LEN - 1);
             tempsite.name[x] = buf[x], x++)
            ;
...
void LoadPAT(char *filename) {
    ...
    char *azfile, *elfile;
    float az, xx, amplitude, rotation, valid1, valid2, delta, azimuth[361],
        azimuth_pattern[361], mechanical_tilt = 0.0, tilt_azimuth, sum;
    double elevation;
    /* elevation_pattern[361][1001]; */ /* 2.7MB. This blows the stack right out
                                           of the water. Allocate on heap. */
    double tilt, tilt_increment, slant_angle[361];
    FILE *fd = NULL;

    float *el_pattern = (float *)malloc(sizeof(float) * 10001);
    unsigned char *read_count =
        (unsigned char *)malloc(sizeof(unsigned char *) * 10001);
    ...
    if (fd != NULL) {
        ...
        if (sum == 0) {
            printf("*** WARNING: There were no radials in the file. This "
                   "probably isn't correct.\n");
        } else if (sum == 1) {
            printf("*** WARNING: There was only one radial in the file. No "
    ...
    if (fd != NULL) {
        ...
        if (sum == 0) {
            printf("*** WARNING: There were no elevation angles in the file. "
                   "This probably isn't correct.\n");
        } else if (sum == 1) {
            printf("*** WARNING: There was only one elevation angle in the "
                   "file. No interpolation was done.\n");
        }
...
char *BZfgets(BZFILE *bzfp, unsigned length) {
    ...
    do {
        if (x == nBuf && bzerror != BZ_STREAM_END && bzerror == BZ_OK &&
            opened) {
...
int LoadSDF(int minlat, int maxlat, int minlon, int maxlon, bool hidef) {
    ...
    if (sdf_path[0] != 0) {
        for (int i = 0; i < known_formats; ++i) {
            snprintf(sdf_file, MAX_PATH_LEN * 2, "%s%d_%d_%d_%d%s.%s", sdf_path,
                     minlat, maxlat, minlon, maxlon, (hidef ? "-hd" : ""),
                     formats[i].suffix);
    ...
    if (!fp) {
        for (int i = 0; i < known_formats; ++i) {
            snprintf(sdf_file, MAX_PATH_LEN * 2, "%d_%d_%d_%d%s.%s", minlat,
                     maxlat, minlon, maxlon, (hidef ? "-hd" : ""),
                     formats[i].suffix);
            if ((fp = fopen(sdf_file, "rb")) != NULL) {
                compressType = formats[i].type;
                break;
...
#ifndef _WIN32
            snprintf(sdf_file, MAX_PATH_LEN * 2, "%d:%d:%d:%d%s.%s", minlat,
                     maxlat, minlon, maxlon, (hidef ? "-hd" : ""),
                     formats[i].suffix);
            if ((fp = fopen(sdf_file, "rb")) != NULL) {
                compressType = formats[i].type;
    ...
    if (!fp && home_sdf_path[0] != 0) {
        for (int i = 0; i < known_formats; ++i) {
            snprintf(sdf_file, MAX_PATH_LEN * 2, "%s%d_%d_%d_%d%s.%s",
                     home_sdf_path, minlat, maxlat, minlon, maxlon,
                     (hidef ? "-hd" : ""), formats[i].suffix);
            if ((fp = fopen(sdf_file, "rb")) != NULL) {
                compressType = formats[i].type;
                break;
            }
#ifndef _WIN32
            snprintf(sdf_file, MAX_PATH_LEN * 2, "%s%d:%d:%d:%d%s.%s",
                     home_sdf_path, minlat, maxlat, minlon, maxlon,
                     (hidef ? "-hd" : ""), formats[i].suffix);
            if ((fp = fopen(sdf_file, "rb")) != NULL) {
                compressType = formats[i].type;
    ...
    if (fp) {
        ...
        fprintf(
            stdout,
            "Can't find file for region %d_%d_%d_%d. Assuming as sea-level.\n",
...
void LoadCities(char *filename) {
    ...
    if (fd != NULL) {
        ...
        while (fd != NULL && feof(fd) == 0) {
            ...
            for (x = 0, y = 0, z = 0;
                 x < MAX_LINE_LEN && input[x] != 0 && z < 3; x++) {
...
void LoadUDT(char *filename) {
    ...
    char input[MAX_LINE_LEN], str[3][MAX_LINE_LEN], tempname[15],
        *pointer = NULL;
    ...
    if (fd1 != NULL) {
        ...
        while (feof(fd1) == 0) {
            ...
            for (i = 0; str[2][i] != 13 && str[2][i] != 10 && str[2][i] != 0;
                 i++)
                ;
            ...
            for (i = 0; str[2][i] != 'M' && str[2][i] != 'm' &&
                        str[2][i] != 0 && i < MAX_LINE_LEN;
                 i++)
                ;
...
int PlotPath(Site source, Site destination, char mask_value) {
    ...
    for (y = 0; y < path->length; y++) {
        ...
        if ((GetMask(path->lat[y], path->lon[y]) & mask_value) == 0) {
            ...
            cos_xmtr_angle = ((rx_alt * rx_alt) + (distance * distance) -
                              (tx_alt * tx_alt)) /
                             (2.0 * rx_alt * distance);
 ...
 * available.
 */
int PlotLRPath(Site source, Site destination, unsigned char mask_value,
               FILE *fd) {
    int x, y, ifs, ofs, errnum;
    char block = 0, strmode[100];
    double loss, azimuth, pattern = 0.0, xmtr_alt, dest_alt, xmtr_alt2,
                          dest_alt2, cos_rcvr_angle, cos_test_angle = 0.0,
                          test_alt, elevation = 0.0, distance = 0.0,
    ...
    for (x = 1; x < path->length - 1; x++)
        elev[x + 2] =
            (path->elevation[x] == 0.0
                 ? (elev_t)(path->elevation[x] * METERS_PER_FOOT)
                 : (elev_t)((clutter + path->elevation[x]) * METERS_PER_FOOT));
    ...
    for (y = 2; (y < (path->length - 1) && path->distance[y] <= max_range);
        ...
        if ((GetMask(path->lat[y], path->lon[y]) & 248) != (mask_value << 3)) {
            ...
            if (destination.amsl_flag)
                ...
                /***/
                dest_alt =
                    four_thirds_earth + destination.alt + path->elevation[y];
            ...
            if (itwom)
                point_to_point(elev, source.alt * METERS_PER_FOOT,
                               ...
                               LR.pol, LR.conf, LR.rel, &loss, strmode,
                               &errnum);
            else
                point_to_point_ITM(elev, source.alt * METERS_PER_FOOT,
                                   ...
                                   LR.radio_climate, LR.pol, LR.conf, LR.rel,
                                   &loss, strmode, &errnum);
            ...
            if (fd != NULL) {
                textlen =
                    snprintf(textout, MAX_LINE_LEN, "%.7f, %.7f, %.3f, %.3f, ",
            ...
            if (fd != NULL && LR.erp == 0.0) {
                textlen += snprintf(textout + textlen, MAX_LINE_LEN - textlen,
                                    "%.2f", loss);
            }
            ...
            if (LR.erp != 0.0) {
                if (dbm) {
                    ...
                    if (fd != NULL) {
                        textlen +=
                            snprintf(textout + textlen, MAX_LINE_LEN - textlen,
                                     "%.3f", dBm);
                    }
                    ...
                    field_strength =
                        (139.4 + (20.0 * log10(LR.frq_mhz)) - loss) +
...
void PlotLOSMap(Site source, double altitude, bool multithread) {
    ...
    fprintf(stdout,
            "\nComputing line-of-sight coverage of \"%s\" with an RX "
            "antenna\nat %.2f %s AGL",
            source.name, (metric ? altitude * METERS_PER_FOOT : altitude),
            (metric ? "meters" : "feet"));
    ...
    for (lon = minwest, x = 0, y = 0; (LonDiff(lon, (double)max_west) <= 0.0);
...
void PlotLRMap(Site source, double altitude, char *plo_filename,
    ...
    fprintf(stdout,
            " contours of \"%s\"\nout to a radius of %.2f %s with an RX "
            "antenna at %.2f %s AGL",
            source.name, (metric ? max_range * KM_PER_MILE : max_range),
    ...
    if (fd == NULL) {
        ...
        for (x = 0; x < region.levels; x++)
            fprintf(fd, "%3d: %3d, %3d, %3d\n", region.level[x],
                    region.color[x][0], region.color[x][1], region.color[x][2]);
        ...
        while (x < 32 && feof(fd) == 0) {
            ...
            ok = sscanf(buf, "%d: %d, %d, %d", &val[0], &val[1], &val[2],
                        &val[3]);
 ...
 */
void WriteImage(char *filename, ImageType imagetype, bool geo, bool kml,
                bool ngs, Site *xmtr, unsigned int txsites) {
    const char *suffix;
    ...
    conversion =
        255.0 / pow((double)(max_elevation - min_elevation), one_over_gamma);
    ...
    try {
        ...
        for (int y = 0, lat = north; y < (int)height;
            ...
            for (int x = 0, lon = max_west; x < (int)width;
                 x++, lon = (double)max_west - (dpp * (double)x)) {
                if (lon < 0.0)
                    lon += 360.0;
 ...
 * degrees from its representation in DEM[][] so that north points up and east
 * points right.
 */
void WriteImageLR(char *filename, ImageType imagetype, bool geo, bool kml,
                  bool ngs, Site *xmtr, unsigned int txsites) {
    const char *suffix;
    ...
    unsigned char mask;
    int indx, x, y, z, colorwidth, x0, y0, loss, level, hundreds, tens, units,
        match;
    DEM *dem;
    ...
    if (kml || geo)
        ...
        south =
            (double)min_north - (30.0 / ppd); /* 30 pixels for bottom legend */
    ...
    if (kml && !geo) {
        ...
        fprintf(
            fd,
            "	 <description>%s Transmitter Path Loss Overlay</description>\n",
            xmtr[0].name);
        fprintf(fd, "	   <GroundOverlay>\n");
        ...
        fprintf(fd, "		  </Icon>\n");
        fprintf(fd, "		  <overlayXY x=\"0\" y=\"1\" "
                    "xunits=\"fraction\" yunits=\"fraction\"/>\n");
        fprintf(fd, "		  <screenXY x=\"0\" y=\"1\" "
                    "xunits=\"fraction\" yunits=\"fraction\"/>\n");
        fprintf(fd, "		  <rotationXY x=\"0\" y=\"0\" "
                    "xunits=\"fraction\" yunits=\"fraction\"/>\n");
        fprintf(fd, "		  <size x=\"0\" y=\"0\" xunits=\"fraction\" "
                    "yunits=\"fraction\"/>\n");
        fprintf(fd, "	   </ScreenOverlay>\n");
        ...
        for (unsigned int i = 0; i < txsites; i++) {
            ...
            fprintf(fd, "		"
                        "<altitudeMode>relativeToGround</altitudeMode>\n");
            fprintf(fd, "		<coordinates>%f,%f,%f</coordinates>\n",
                    (xmtr[i].lon < 180.0 ? -xmtr[i].lon : 360.0 - xmtr[i].lon),
                    xmtr[i].lat, xmtr[i].alt);
            fprintf(fd, "	  </Point>\n");
            fprintf(fd, "	 </Placemark>\n");
        }
    ...
    fprintf(stdout, "\nWriting LR map \"%s\" (%ux%u image)... ", mapfile,
            imgwidth, imgheight);
    fflush(stdout);
            ...
            for (x = 0, lon = max_west; x < (int)width;
                ...
                if (dem) {
                    ...
                    if (mask & 2) {
                        ...
                        if (loss == 0 || (contour_threshold != 0 &&
                                          loss > abs(contour_threshold))) {
                            if (ngs) /* No terrain */
                                pixel = COLOR_WHITE;
                            else {
 ...
 */
void WriteImageSS(char *filename, ImageType imagetype, bool geo, bool kml,
                  bool ngs, Site *xmtr, unsigned int txsites) {
    const char *suffix;
    char *mapfile, *geofile, *kmlfile, *ckfile;
#if DO_KMZ
    ...
    unsigned int imgheight, imgwidth;
    unsigned char mask;
    int indx, x, y, z = 1, x0, y0, signal, level, hundreds, tens, units, match,
                    colorwidth;
    DEM *dem;
 ...
 */
void WriteImageDBM(char *filename, ImageType imagetype, bool geo, bool kml,
                   bool ngs, Site *xmtr, unsigned int txsites) {
    const char *suffix;
    char *mapfile, *geofile, *kmlfile, *ckfile;
    unsigned width, height, terrain, red, green, blue;
    ...
    unsigned char mask;
    DEM *dem;
    int indx, x, y, z = 1, x0, y0, dBm, level, hundreds, tens, units, match,
                    colorwidth;
    double conversion, one_over_gamma, lat, lon, north, south, east, west,
        minwest;
    FILE *fd;
    ...
    if (kml && !geo) {
        ...
        fprintf(
            fd,
            "		 <name>SPLAT! Signal Power Level Contours</name>\n");
        fprintf(fd, "		 <description>SPLAT! Coverage</description>\n");
        fprintf(
            fd,
            "		 <color>8cffffff</color>\n"); /* transparency level,
...
void GraphTerrain(Site source, Site destination, char *name) {
    ...
    fprintf(fd, "set yrange [%2.3f to %2.3f]\n",
            metric ? minheight * METERS_PER_FOOT : minheight,
            metric ? maxheight * METERS_PER_FOOT : maxheight);
    fprintf(fd, "set encoding iso_8859_1\n");
    fprintf(fd, "set term %s\n", term);
    fprintf(
        fd,
        "set title \"%s Terrain Profile Between %s and %s (%.2f%c Azimuth)\"\n",
        SPLAT_NAME, destination.name, source.name, Azimuth(destination, source),
        176);
    ...
    if (clutter > 0.0) {
        if (metric)
            fprintf(fd,
                    "plot \"profile.gp\" title \"Terrain Profile\" with lines, "
                    "\"clutter.gp\" title \"Clutter Profile (%.2f meters)\" "
                    "with lines\n",
                    clutter * METERS_PER_FOOT);
        else
            fprintf(fd,
                    "plot \"profile.gp\" title \"Terrain Profile\" with lines, "
                    "\"clutter.gp\" title \"Clutter Profile (%.2f feet)\" with "
                    "lines\n",
                    clutter);
    }
...
void GraphElevation(Site source, Site destination, char *name) {
    ...
    fprintf(fd,
            ...
            "azimuth)\"\n",
            SPLAT_NAME, destination.name, source.name,
    ...
    if (clutter > 0.0) {
        if (metric)
            fprintf(fd,
                    "plot \"profile.gp\" title \"Real Earth Profile\" with "
                    "lines, \"clutter.gp\" title \"Clutter Profile (%.2f "
                    "meters)\" with lines, \"reference.gp\" title \"Line of "
                    "Sight Path (%.2f%c elevation)\" with lines\n",
                    clutter * METERS_PER_FOOT, refangle, 176);
        else
            fprintf(fd,
                    "plot \"profile.gp\" title \"Real Earth Profile\" with "
                    "lines, \"clutter.gp\" title \"Clutter Profile (%.2f "
                    "feet)\" with lines, \"reference.gp\" title \"Line of "
                    "Sight Path (%.2f%c elevation)\" with lines\n",
        ...
        fprintf(fd,
                "plot \"profile.gp\" title \"Real Earth Profile\" with lines, "
                "\"reference.gp\" title \"Line of Sight Path (%.2f%c "
                "elevation)\" with lines\n",
                refangle, 176);
 ...
 * If no extension is found, .png is assumed.
 */
void GraphHeight(Site source, Site destination, char *name, bool fresnel_plot,
                 bool normalized) {
    int x;
    char basename[MAX_PATH_LEN], term[30], ext[20];
    double a, b, c, height = 0.0, refangle, cangle, maxheight = -100000.0,
                    ...
                    r = 0.0, d = 0.0, d1 = 0.0, terrain, azimuth, distance,
                    dheight = 0.0, minterrain = 100000.0, minearth = 100000.0,
                    miny, maxy, min2y, max2y;
    Site remote;
    ...
    for (x = 0; x < path->length - 1; x++) {
        ...
        c = b * sin(refangle * DEG2RAD + HALFPI) /
            sin(HALFPI - refangle * DEG2RAD - cangle);
        ...
        if (normalized) {
            ...
            if ((LR.frq_mhz >= 20.0) && (LR.frq_mhz <= 20000.0) &&
                fresnel_plot) {
                f_zone += r;
                fpt6_zone += r;
            }
    ...
    fprintf(fd, "set grid\n");
    fprintf(fd, "set yrange [%2.3f to %2.3f]\n",
            metric ? miny * METERS_PER_FOOT : miny,
            metric ? maxy * METERS_PER_FOOT : maxy);
    fprintf(fd, "set y2range [%2.3f to %2.3f]\n",
            metric ? min2y * METERS_PER_FOOT : min2y,
            metric ? max2y * METERS_PER_FOOT : max2y);
    ...
    if ((LR.frq_mhz >= 20.0) && (LR.frq_mhz <= 20000.0) && fresnel_plot)
        fprintf(fd,
                "set title \"%s Path Profile Between %s and %s (%.2f%c "
                "azimuth)\\nWith First Fresnel Zone\"\n",
                SPLAT_NAME, destination.name, source.name, azimuth, 176);

    else
        fprintf(fd,
                "set title \"%s Height Profile Between %s and %s (%.2f%c "
                "azimuth)\"\n",
                SPLAT_NAME, destination.name, source.name, azimuth, 176);
    ...
    if (normalized) {
        if (metric)
            fprintf(fd,
                    "set ylabel \"Normalized Height Referenced To LOS Path "
                    "Between\\n%s and %s (meters)\"\n",
                    destination.name, source.name);
    ...
    if ((LR.frq_mhz >= 20.0) && (LR.frq_mhz <= 20000.0) && fresnel_plot) {
        if (clutter > 0.0) {
            if (metric)
                fprintf(fd,
                        "plot \"profile.gp\" title \"Point-to-Point Profile\" "
                        "with lines, \"clutter.gp\" title \"Ground Clutter "
                        "(%.2f meters)\" with lines, \"reference.gp\" title "
                        "\"Line of Sight Path\" with lines, \"curvature.gp\" "
                        "axes x1y2 title \"Earth's Curvature Contour\" with "
                        "lines, \"fresnel.gp\" axes x1y1 title \"First Fresnel "
                        "Zone (%.3f MHz)\" with lines, \"fresnel_pt_6.gp\" "
                        "title \"%.0f%% of First Fresnel Zone\" with lines\n",
                        clutter * METERS_PER_FOOT, LR.frq_mhz,
                        fzone_clearance * 100.0);
            else
                fprintf(fd,
                        "plot \"profile.gp\" title \"Point-to-Point Profile\" "
                        "with lines, \"clutter.gp\" title \"Ground Clutter "
                        "(%.2f feet)\" with lines, \"reference.gp\" title "
                        "\"Line of Sight Path\" with lines, \"curvature.gp\" "
                        "axes x1y2 title \"Earth's Curvature Contour\" with "
                        "lines, \"fresnel.gp\" axes x1y1 title \"First Fresnel "
                        "Zone (%.3f MHz)\" with lines, \"fresnel_pt_6.gp\" "
            ...
            fprintf(
                fd,
                "plot \"profile.gp\" title \"Point-to-Point Profile\" with "
                "lines, \"reference.gp\" title \"Line of Sight Path\" with "
                "lines, \"curvature.gp\" axes x1y2 title \"Earth's Curvature "
                "Contour\" with lines, \"fresnel.gp\" axes x1y1 title \"First "
                "Fresnel Zone (%.3f MHz)\" with lines, \"fresnel_pt_6.gp\" "
                "title \"%.0f%% of First Fresnel Zone\" with lines\n",
        ...
        if (clutter > 0.0) {
            if (metric)
                fprintf(
                    fd,
                    "plot \"profile.gp\" title \"Point-to-Point Profile\" with "
                    "lines, \"clutter.gp\" title \"Ground Clutter (%.2f "
                    "meters)\" with lines, \"reference.gp\" title \"Line Of "
                    "Sight Path\" with lines, \"curvature.gp\" axes x1y2 title "
                    "\"Earth's Curvature Contour\" with lines\n",
                    clutter * METERS_PER_FOOT);
            else
                fprintf(
                    fd,
            ...
            fprintf(fd, "plot \"profile.gp\" title \"Point-to-Point Profile\" "
                        "with lines, \"reference.gp\" title \"Line Of Sight "
                        "Path\" with lines, \"curvature.gp\" axes x1y2 title "
                        "\"Earth's Curvature Contour\" with lines\n");
    }
...
void ObstructionAnalysis(Site xmtr, Site rcvr, double f, FILE *outfile) {
    ...
    for (x = path->length - 1; x > 0; x--) {
        ...
        if (cos_tx_angle > cos_test_angle) {
            if (h_r == h_r_orig)
                fprintf(outfile,
                        "Between %s and %s, %s detected obstructions at:\n\n",
                        rcvr.name, xmtr.name, SPLAT_NAME);

            if (site_x.lat >= 0.0) {
                if (metric)
                    fprintf(outfile,
                            "   %8.4f N,%9.4f W, %5.2f kilometers, %6.2f "
                            "meters AMSL\n",
                    ...
                    fprintf(
                        ...
                        site_x.lat, site_x.lon, d_x / 5280.0,
                        h_x - earthradius);
            }
                ...
                if (metric)
                    fprintf(outfile,
                            "   %8.4f S,%9.4f W, %5.2f kilometers, %6.2f "
                            "meters AMSL\n",
                            -site_x.lat, site_x.lon,
                            KM_PER_MILE * (d_x / 5280.0),
                            METERS_PER_FOOT * (h_x - earthradius));
        ...
        while (cos_tx_angle > cos_test_angle) {
            ...
            cos_test_angle =
                ((h_r * h_r) + (d_x * d_x) - (h_x * h_x)) / (2.0 * h_r * d_x);
            cos_tx_angle = ((h_r * h_r) + (d_tx * d_tx) - (h_t * h_t)) /
                           (2.0 * h_r * d_tx);
        }
        ...
        if (f) {
            ...
            cos_tx_angle_f1 =
                ...
                (2.0 * h_r_f1 * d_tx);
            h_los = sqrt(h_r_f1 * h_r_f1 + d_x * d_x -
            ...
            cos_tx_angle_fpt6 =
                ...
                (2.0 * h_r_fpt6 * d_tx);
            h_los = sqrt(h_r_fpt6 * h_r_fpt6 + d_x * d_x -
                         2 * h_r_fpt6 * d_x * cos_tx_angle_fpt6);
            h_f = h_los -
                  fzone_clearance * sqrt(lambda * d_x * (d_tx - d_x) / d_tx);
    ...
    if (h_r > h_r_orig) {
        if (metric)
            snprintf(buf, MAX_PATH_LEN,
                     "\nAntenna at %s must be raised to at least %.2f meters "
            ...
            snprintf(buf, MAX_PATH_LEN,
                     "\nAntenna at %s must be raised to at least %.2f feet "
                     "AGL\nto clear all obstructions detected by %s.\n",
        ...
        if (h_r_fpt6 > h_r_orig) {
            if (metric)
                ...
                snprintf(buf_fpt6, MAX_PATH_LEN,
                         "\nAntenna at %s must be raised to at least %.2f feet "
                         "AGL\nto clear %.0f%c of the first Fresnel zone.\n",
                         rcvr.name, h_r_fpt6 - GetElevation(rcvr) - earthradius,
                         fzone_clearance * 100.0, 37);
        }
        ...
        if (h_r_f1 > h_r_orig) {
            if (metric)
                snprintf(buf_f1, MAX_PATH_LEN,
                         "\nAntenna at %s must be raised to at least %.2f "
 ...
 * If (graph_it == true), then gnuplot is invoked to generate an appropriate
 * output file indicating the ITM/ITWOM model loss between the source and
 * destination locations. You don't get a choice in the name of the text report,
 * but "name" is the name assigned to the output file generated by gnuplot. The
 * filename extension is used to set gnuplot's terminal setting and output file
 * type. If no extension is found, .png is assumed.
 */
void PathReport(Site source, Site destination, char *name, bool graph_it) {
    ...
    double maxloss = -100000.0, minloss = 100000.0, loss, haavt, angle1, angle2,
           azimuth, pattern = 1.0, patterndB = 0.0, total_loss = 0.0,
           cos_xmtr_angle, cos_test_angle = 0.0, source_alt, test_alt, dest_alt,
           source_alt2, dest_alt2, distance, elevation, four_thirds_earth,
           field_strength, free_space_loss = 0.0, eirp = 0.0, voltage, rxp, dBm,
           power_density;
    FILE *fd = NULL, *fd2 = NULL;

    snprintf(report_name, MAX_PATH_LEN, "%s-to-%s.txt", source.name,
             destination.name);
    ...
    if (source.lat >= 0.0) {
        fprintf(fd2, "Site location: %.4f North / %.4f West", source.lat,
                source.lon);
        ...
        fprintf(fd2, "Site location: %.4f South / %.4f West", -source.lat,
                source.lon);
    ...
    if (destination.lat >= 0.0) {
        fprintf(fd2, "Site location: %.4f North / %.4f West", destination.lat,
                destination.lon);
        fprintf(fd2, " (%s N / ", dec2dms(destination.lat));
    }

    else {
        fprintf(fd2, "Site location: %.4f South / %.4f West", -destination.lat,
                destination.lon);
    ...
    if (LR.frq_mhz > 0.0) {
        if (itwom)
            fprintf(fd2,
                    "ITWOM Version %.1f Parameters Used In This Analysis:\n\n",
                    ITWOMVersion());
        else
        ...
        if (patterndB != 0.0) {
            if (patterndB == -9999) {
                fprintf(fd2,
                        "%s antenna pattern towards %s: -infinity (blocked)\n",
        ...
        for (x = 1; x < path->length - 1; x++)
            elev[x + 2] = METERS_PER_FOOT *
                          (path->elevation[x] == 0.0
                               ? (elev_t)(path->elevation[x])
                               : (elev_t)(clutter + path->elevation[x]));
        ...
        for (y = 2; y < (path->length - 1);
             y++) /* path->length-1 avoids LR error */
        {
            distance = 5280.0 * path->distance[y];
            ...
            if (got_elevation_pattern) {
                ...
                for (x = 2, block = 0; x < y && block == 0; x++) {
                    ...
                    cos_test_angle = ((source_alt2) + (distance * distance) -
                                      (test_alt * test_alt)) /
                                     (2.0 * source_alt * distance);
        ...
        if (distance != 0.0) {
            free_space_loss =
                36.6 + (20.0 * log10(LR.frq_mhz)) + (20.0 * log10(distance));
        ...
        if (LR.erp != 0.0) {
            field_strength = (139.4 + (20.0 * log10(LR.frq_mhz)) - total_loss) +
                             (10.0 * log10(LR.erp / 1000.0));
            ...
            dBm = 10.0 * (log10(rxp * 1000.0));
            power_density =
                (eirp / (pow(10.0, (total_loss - free_space_loss) / 10.0)));
            ...
            voltage =
                1.0e6 *
                sqrt(50.0 * (eirp / (pow(10.0, (total_loss - 2.14) / 10.0))));
            fprintf(
                fd2,
                "Voltage across a 50 ohm dipole at %s: %.2f uV (%.2f dBuV)\n",
                destination.name, voltage, 20.0 * log10(voltage));

            voltage =
                1.0e6 *
                sqrt(75.0 * (eirp / (pow(10.0, (total_loss - 2.14) / 10.0))));
            fprintf(
                fd2,
                "Voltage across a 75 ohm dipole at %s: %.2f uV (%.2f dBuV)\n",
                destination.name, voltage, 20.0 * log10(voltage));
        }
    ...
    if (graph_it) {
        ...
        if (metric)
            ...
            fprintf(
                fd, "set xlabel \"Distance Between %s and %s (%.2f miles)\"\n",
...
void SiteReport(Site xmtr) {
    ...
    if (xmtr.lat >= 0.0) {
        fprintf(fd, "Site location: %.4f North / %.4f West", xmtr.lat,
                xmtr.lon);
        fprintf(fd, " (%s N / ", dec2dms(xmtr.lat));
    }

    else {
        fprintf(fd, "Site location: %.4f South / %.4f West", -xmtr.lat,
                xmtr.lon);
    ...
    if (metric) {
        if (xmtr.amsl_flag)
            ...
            fprintf(fd, "Antenna height: %.2f meters AGL / %.2f meters AMSL\n",
                    METERS_PER_FOOT * xmtr.alt,
                    METERS_PER_FOOT * (xmtr.alt + GetElevation(xmtr)));
        }
    }
...
int LoadANO(char *filename) {
    ...
    if (fd != NULL) {
        ...
        fgets(buf, MAX_LINE_LEN, fd);
        sscanf(buf, "%lf, %lf, %lf, %lf, %lf", &latitude, &longitude, &azimuth,
               &elevation, &ano);
        ...
        while (feof(fd) == 0) {
            if (LR.erp == 0.0) {
                ...
                if (contour_threshold == 0 ||
                    (fabs(ano) <= (double)contour_threshold)) {
                    ano = fabs(ano);
            ...
            fgets(buf, MAX_LINE_LEN, fd);
            sscanf(buf, "%lf, %lf, %lf, %lf, %lf", &latitude, &longitude,
...
void WriteKML(Site source, Site destination) {
    ...
    snprintf(report_name, MAX_PATH_LEN - 4, "%s-to-%s", source.name,
             destination.name);
...
int main(int argc, char *argv[]) {
    ...
    char buf[MAX_PATH_LEN], rxfile[MAX_PATH_LEN], txfile[MAX_PATH_LEN],
        boundary_file[5][MAX_PATH_LEN], udt_file[MAX_PATH_LEN],
        ani_filename[MAX_PATH_LEN], ano_filename[MAX_PATH_LEN],
        logfile[MAX_PATH_LEN];

    char *env = NULL;
    char rxsite = 0;

    double altitude = 0.0, altitudeLR = 0.0, tx_range = 0.0, rx_range = 0.0,
           deg_range = 0.0, deg_limit = 0.0, deg_range_lon, er_mult;
    ...
    if (argc == 1) {
        fprintf(stdout, "\n\t\t --==[ %s%s v%s Available Options... ]==--\n\n",
                SPLAT_NAME, (appmode == APPMODE_HD ? " HD" : ""),
                SPLAT_VERSION);
        ...
        fprintf(stdout, "       -c plot LOS coverage of TX(s) with an RX "
                        "antenna at X feet/meters AGL\n");
        fprintf(stdout, "       -L plot path loss map of TX based on an RX at "
                        "X feet/meters AGL\n");
        fprintf(
            stdout,
            "       -s filename(s) of city/site file(s) to import (5 max)\n");
        fprintf(stdout, "       -b filename(s) of cartographic boundary "
                        "file(s) to import (5 max)\n");
        fprintf(stdout,
        ...
        fprintf(stdout, "       -o filename of topographic map to generate "
                        "(without suffix)\n");
        fprintf(stdout,
                "       -u filename of user-defined terrain file to import\n");
        fprintf(stdout, "       -d sdf file directory path (overrides path in "
                        "~/.splat_path file)\n");
        fprintf(stdout, "       -m earth radius multiplier\n");
        fprintf(stdout, "       -n do not plot LOS paths in output maps\n");
        fprintf(stdout, "       -N do not produce unnecessary site or "
                        "obstruction reports\n");
        fprintf(stdout,
        ...
        fprintf(stdout, "       -v N verbosity level. Default is 1. Set to 0 "
                        "to quiet everything.\n");
        ...
        fprintf(stdout, "      -db threshold beyond which contours will not be "
                        "displayed\n");
        fprintf(stdout,
        ...
        fprintf(stdout, "     -jpg when generating maps, create jpgs instead "
                        "of pngs or ppms\n");
#ifdef HAVE_LIBPNG
        fprintf(stdout, "     -ppm when generating maps, create ppms instead "
                        "of pngs or jpgs\n");
#endif
        fprintf(
            stdout,
        ...
        fprintf(stdout, "     -geo generate an Xastir .geo georeference file "
                        "(with .ppm output)\n");
        fprintf(stdout, "     -dbm plot signal power level contours rather "
                        "than field strength\n");
        fprintf(stdout,
                "     -log copy command line string to this output file\n");
        fprintf(stdout, "   -gpsav preserve gnuplot temporary working files "
                        "after SPLAT! execution\n");
        fprintf(
            stdout,
            "   -itwom invoke the ITWOM model instead of using Longley-Rice\n");
        fprintf(stdout, "  -metric employ metric rather than imperial units "
                        "for all user I/O\n");
        ...
        fprintf(stdout,
                "This compilation of %s supports analysis over a region of "
                "%dx%d square degree%s.\n",
                SPLAT_NAME, maxpagesides, maxpagesides,
                (maxpagesides > 1) ? "s" : "");

        unsigned long pathmem;
        unsigned long demmem;
        pathmem = (WorkQueue::maxWorkers() *
                   SizeofPath(APPMODE_NORMAL, maxpagesides)) /
                  1024;
        demmem =
            (maxpagesides * maxpagesides * SizeofDEM(APPMODE_NORMAL)) / 1024;
        fprintf(stdout,
                "At the %dx%d maximum in normal mode it will require %lu MB of "
                "RAM.\n",
        ...
        pathmem =
            (WorkQueue::maxWorkers() * SizeofPath(APPMODE_HD, maxpagesides)) /
            1024;
        demmem = (maxpagesides * maxpagesides * SizeofDEM(APPMODE_HD)) / 1024;
        fprintf(
            stdout,
            "At the %dx%d maximum in HD mode it will require %lu MB of RAM.\n",
            maxpagesides, maxpagesides, (pathmem + demmem) / 1024);
        fprintf(stdout, "Each Path can be up to %lu KB.\n\n", pathmem);
        /*
           fprintf(stdout,"Each DEM: %lu KB\n", demmem);
    ...
    for (int x = 1, z = 0; x < argc; x++) {
        ...
        if (strcmp(argv[x], "-t") == 0) {
            ...
            while (z < argc && argv[z][0] && argv[z][0] != '-' &&
                   txsites < 30) {
    ...
    sprintf(header, "\n\t\t--==[ Welcome To %s%s v%s ]==--\n\n", SPLAT_NAME,
            (appmode == APPMODE_HD ? " HD" : ""), SPLAT_VERSION);
    ...
    for (unsigned int x = 0; x < txsites; x++) {
        if (tx_site[x].lat == 91.0 && tx_site[x].lon == 361.0) {
            fprintf(stderr, "\n*** ERROR: Transmitter site #%d not found!",
                    x + 1);
            errcount++;
        }
    }
    ...
    if ((coverage + LRmap + ani_filename[0]) == 0 && rx_site.lat == 91.0 &&
        rx_site.lon == 361.0) {
    ...
    if (ani_filename[0]) {
        ...
        if (LR.erp == 0.0)
            ...
            if (dbm)
                WriteImageDBM(mapfile, imagetype, geo, kml, ngs, tx_site,
                              txsites);
            else
                WriteImageSS(mapfile, imagetype, geo, kml, ngs, tx_site,
                             txsites);
        }
    ...
    if (pt2pt_mode) {
        char longley_ext[4], terrain_ext[4], elevation_ext[4], height_ext[4],
            norm_height_ext[4];
        ...
        for (unsigned int x = 0; x < txsites && x < 4; x++) {
            ...
            if (nositereports == 0) {
                if (txsites > 1)
                    snprintf(buf, MAX_PATH_LEN, "%s-%d.%s", longley_file, x,
                             longley_ext);
                else
                    snprintf(buf, MAX_PATH_LEN, "%s.%s", longley_file,
                             longley_ext);
            ...
            if (terrain_plot) {
                if (txsites > 1)
                    snprintf(buf, MAX_PATH_LEN, "%s-%d.%s", terrain_file, x,
                             terrain_ext);
                else
                    snprintf(buf, MAX_PATH_LEN, "%s.%s", terrain_file,
                             terrain_ext);
            ...
            if (elevation_plot) {
                if (txsites > 1)
                    snprintf(buf, MAX_PATH_LEN, "%s-%d.%s", elevation_file, x,
                             elevation_ext);
                else
                    snprintf(buf, MAX_PATH_LEN, "%s.%s", elevation_file,
                             elevation_ext);
            ...
            if (height_plot) {
                if (txsites > 1)
                    snprintf(buf, MAX_PATH_LEN, "%s-%d.%s", height_file, x,
                             height_ext);
                else
                    snprintf(buf, MAX_PATH_LEN, "%s.%s", height_file,
                             height_ext);
            ...
            if (norm_height_plot) {
                if (txsites > 1)
                    snprintf(buf, MAX_PATH_LEN, "%s-%d.%s", norm_height_file, x,
                             norm_height_ext);
                else
                    snprintf(buf, MAX_PATH_LEN, "%s.%s", norm_height_file,
                             norm_height_ext);
    ...
    if (map || topomap) {
        ...
        if (coverage || pt2pt_mode || topomap)
            ...
            if (LR.erp == 0.0)
                WriteImageLR(mapfile, imagetype, geo, kml, ngs, tx_site,
                             txsites);
            else if (dbm)
                WriteImageDBM(mapfile, imagetype, geo, kml, ngs, tx_site,
                              txsites);
