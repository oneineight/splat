 ...
 *Further test version of itwom2.0m re adj to Hrzn range factors  *
...
double saalos(double d, prop_type &prop, propa_type &propa) {
    double ensa, encca, q, dp, dx, tde, hc, ucrpc, ctip, tip, tic = 0.0, stic,
                                                              ctic, sta;
    double ttc, cttc = 0.0, crpc = 0.0, ssnps = 0.0, d1a, rsp, tsp, arte, zi,
                pd, pdk, hone, tvsr;
    double saalosv = 0.0;
    ...
    if (d == 0.0) {
        ...
        if (prop.tgh > prop.cch) /* for TX ant above all clutter height*/
        {
            ...
            if (d1a < 50.0) {
                ...
                if (d1a < 225.0) {
                    ...
                    if (pdk > zi) {
                        q = (pdk - zi) * 10.2 *
                            ((sqrt(mymax(0.01, log10(prop.wn * 47.7) - 2.0))) /
                             (100 - zi));
                    } else {
                        q = ((zi - pdk) / zi) *
                            (-20.0 * mymax(0.01, log10(prop.wn * 47.7) - 2.0)) /
                            sqrt(hone);
                    }
                    arte = arte + q;
                }
        ...
        } else /* for TX at or below clutter height */
        {
            ...
            arte = arte - (mymax(0.01, log10(prop.wn * 47.7) - 2)) *
                              (prop.hg[1] / prop.tgh);
        }
        saalosv = arte;
    }
...
double adiff(double d, prop_type &prop, propa_type &propa) {
    ...
    if (d == 0) {
        ...
        q = (wd1 + xd1 / d) *
            mymin(((1.0 - 0.8 * exp(-d / 50e3)) * prop.dh * prop.wn), 6283.2);
        wd = 25.1 / (25.1 + sqrt(q));
...
double adiff2(double d, prop_type &prop, propa_type &propa) {
    complex<double> prop_zgnd(prop.zgndreal, prop.zgndimag);
    static double wd1, xd1, qk, aht, xht, toh, toho, roh, roho, dto, dto1, dtro,
        dro, dro2, drto, dtr, dhh1, dhh2, /* dhec, */ dtof, dto1f, drof, dro2f;
    double a, q, pk, rd, ds, dsl, /* dfdh, */ th, wa, /* ar, wd, sf1, */ sf2,
        /* ec, */ vv, kedr = 0.0, arp = 0.0, sdr = 0.0, pd = 0.0, srp = 0.0,
                      kem = 0.0, csd = 0.0, sdl = 0.0, adiffv2 = 0.0,
                      closs = 0.0;
        ...
        if (int(ds) > 0) /* there are 2 obstacles */
        {
            if (int(prop.dl[1]) > 0.0) /* receive site past 2nd peak */
            {
                ...
                if (prop.the[1] < 0.2) /* receive grazing angle below 0.2 rad */
                {
                    ...
                    if (prop.hht <
                        3400) /* if below tree line, foliage top loss */
                    {
            ...
            if (int(prop.dl[1]) > 0.0) /* receive site past 1st peak */
            {
                    ...
                    0.2) /* receive grazing angle less than .2 radians */
                {
                    ...
                    if (prop.hht < 3400) {
                        ...
                        if (pd >= 3.141592654) {
                            pd = 6.283185307 - pd;
                            csd = abq_alos(complex<double>(sdl, 0) +
                                           complex<double>(kem * -cos(pd),
                                                           kem * -sin(pd)));
                        } else {
                            csd = abq_alos(
                                complex<double>(sdl, 0) +
                                complex<double>(kem * cos(pd), kem * sin(pd)));
                        }
...
double alos(double d, prop_type &prop, propa_type &propa) {
    ...
    if (d == 0.0) {
        ...
        alosv = (-4.343 * log(abq_alos(complex<double>(cos(q), -sin(q)) + r)) -
                 alosv) *
                    wls +
                alosv;
    }
    ...
    if (d == 0.0) {
        ...
        if (prop.mdp < 0) {
            ...
            if ((sps < 0.05) && (prop.cch > hrg) &&
                (prop.dist < prop.dl[0])) /* if far from transmitter and
                                             receiver below canopy */
            {
    ...
    if (prop.dist < propa.dlsa) {
        ...
        } else /* for ITWOM point-to-point mode */
        {
            ...
            if (prop.los == 1) /* if line of sight */
            {
                ...
                } else if (int(prop.dist - prop.dl[0]) >
...
double avar(double zzt, double zzl, double zzc, prop_type &prop,
            propv_type &propv) {
    static int kdv;
    static double dexa, de, vmd, vs0, sgl, sgtm, sgtp, sgtd, tgtd, gm, gp, cv1,
    ...
    double xv1[7] = {144.9e3, 228.9e3, 262.6e3, 84.1e3,
                     228.9e3, 141.7e3, 2222.e3};
    double xv2[7] = {190.3e3, 205.2e3, 185.2e3, 101.1e3,
                     205.2e3, 315.9e3, 164.8e3};
    double xv3[7] = {133.8e3, 143.6e3, 99.8e3, 98.6e3,
    ...
    double xsm1[7] = {762.2e3, 100.4e3, 138.2e3, 139.1e3,
                      93.7e3,  187.8e3, 609.8e3};
    double xsm2[7] = {123.6e3, 172.5e3, 242.2e3, 132.7e3,
                      186.8e3, 169.6e3, 119.9e3};
    double xsm3[7] = {94.5e3,  136.4e3, 178.6e3, 193.5e3,
    ...
    double xsp1[7] = {636.9e3, 138.7e3, 165.3e3, 464.4e3,
                      93.2e3,  216.0e3, 136.2e3};
    double xsp2[7] = {134.8e3, 143.7e3, 225.7e3, 93.1e3,
                      135.9e3, 152.0e3, 188.5e3};
    double xsp3[7] = {95.6e3,  98.6e3,  129.7e3, 94.2e3,
    ...
    if (np >= 2) {
        ...
        if (!wq) {
            for (i = 1; i < np; i++) {
                sb -= xi;
                q = pfl[np + 2 - i] -
                    (qc * (prop.dist - sb) + prop.the[1]) * (prop.dist - sb) -
                    zb;
                if (q > 0.0) {
...
void qlrpfl(double pfl[], int klimx, int mdvarx, prop_type &prop,
    ...
    if (prop.dl[0] + prop.dl[1] > 1.5 * prop.dist) {
                                   ...
                                   to adjust grazing angles */
        {
            q = sqrt(2.0 * prop.he[j] / prop.gme);
            prop.the[j] =
                (0.65 * prop.dh * (q / prop.dl[j] - 1.0) - 2.0 * prop.he[j]) /
                q;
        }
    }
...
void qlrpfl2(double pfl[], int klimx, int mdvarx, prop_type &prop,
    ...
    if ((np < 1) || (pfl[1] > 150.0)) {
        /* for TRANSHORIZON; diffraction over a mutual horizon, or for one or
         * more obstructions */
        ...
        /* for a Line-of-Sight path */
            ...
            /* for one or more obstructions only NOTE buried as in ITM FORTRAN
             * and DLL, not functional  */
...
double deg2rad(double d) { return d * 3.1415926535897 / 180.0; }
...
/******************************************************************************
        ...
        clutter_height  	25.2 meters for compatibility with ITU-R
P.1546-2.

        clutter_density 	1.0 for compatibility with ITU-R P.1546-2.

        delta_h_diff		optional delta h for beyond line of sight. 90 m.
average. setting to 0.0 will default to use of original internal use of delta-h
for beyond line-of-sight range.

        mode_var		set to 12; or to 1 for FCC ILLR;  see
documentation

        enc_ncc_clcref 		clutter refractivity; 1000 N-units to match
ITU-R P.1546-2

        eno=eno_ns_surfref	atmospheric refractivity at sea level; 301
N-units nominal (ranges from 250 for dry, hot day to 450 on hot, humid day]
...
Tropical, 4-Desert, 5-Continental Temperate, 6-Maritime Temperate, Over Land,
                        7-Maritime Temperate, Over Sea
         timepct, locpct, confpct: .01 to .99
         elev[]: [num points - 1], [delta dist(meters)], [height(meters) point
1], ..., [height(meters) point n] propmode:  Value   Mode -1     mode is
undefined 0     Line of Sight 5     Single Horizon, Diffraction 6     Single
Horizon, Troposcatter 9     Double Horizon, Diffraction 10     Double Horizon,
Troposcatter errnum: 0- No Error. 1- Warning: Some parameters are nearly out of
range. Results should be used with caution. 2- Note: Default parameters have
been substituted for impossible ones. 3- Warning: A combination of parameters is
out of range. Results are probably invalid. Other-  Warning: Some parameters are
out of range. Results are probably invalid.
*************************************************************************************************/
{
...
Tropical, 4-Desert, 5-Continental Temperate, 6-Maritime Temperate, Over Land,
         ...
         conf, rel: .01 to .99
         elev[]: [num points - 1], [delta dist(meters)], [height(meters) point
1], ..., [height(meters) point n] errnum: 0- No Error. 1- Warning: Some
parameters are nearly out of range. Results should be used with caution. 2-
Note: Default parameters have been substituted for impossible ones. 3- Warning:
A combination of parameters is out of range. Results are probably invalid.
                 Other-  Warning: Some parameters are out of range.
                                  Results are probably invalid.
*************************************************************************************************/
{
    ...
    dbloss =
        avar(zr, 0.0, zc, prop, propv) + fs; // avar(time,location,confidence)
    errnum = prop.kwx;
}
...
void area(long ModVar, double deltaH, double tht_m, double rht_m,
    ...
    // radio_climate: 1-Equatorial, 2-Continental Subtropical, 3-Maritime
    // Tropical,
    //                4-Desert, 5-Continental Temperate, 6-Maritime Temperate,
    //                Over Land, 7-Maritime Temperate, Over Sea
    // ModVar: 0 - Single: pctConf is "Time/Situation/Location", pctTime, pctLoc
    // not used
    ...
    //         not used 3 - Broadcast: pctTime is "Time", pctLoc is "Location",
    ...
    //         ones. 3- Warning: A combination of parameters is out of range.
    //                     Results are probably invalid.
    //         Other-  Warning: Some parameters are out of range.
    //                          Results are probably invalid.
