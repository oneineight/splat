    ...
    if (sr.clutter > 0.0) {
        if (sr.metric)
            fprintf(fd,
                    ...
                    "\"clutter.gp\" title \"sr.clutter Profile (%.2f meters)\" "
                    "with lines\n",
                    sr.clutter * METERS_PER_FOOT);
        else
            fprintf(fd,
                    "plot \"profile.gp\" title \"Terrain Profile\" with lines, "
                    "\"clutter.gp\" title \"sr.clutter Profile (%.2f feet)\" "
                    "with lines\n",
                    sr.clutter);
    }
    ...
    if (sr.clutter > 0.0) {
        if (sr.metric)
            fprintf(fd,
                    ...
                    "lines, \"clutter.gp\" title \"sr.clutter Profile (%.2f "
...
void GnuPlot::GraphHeight(const Site &source, const Site &destination,
    ...
    for (x = 0; x < path.length - 1; x++) {
        ...
        if (normalized) {
            ...
            if ((lrp.frq_mhz >= 20.0) && (lrp.frq_mhz <= 20000.0) &&
                fresnel_plot) {
                f_zone += r;
                fpt6_zone += r;
            }
        ...
        if (sr.metric) {
            ...
            if (fd1 != NULL && x > 0 && x < path.length - 2)
                fprintf(fd1, "%f\t%f\n", KM_PER_MILE * path.distance[x],
                        METERS_PER_FOOT *
                            (terrain == 0.0 ? height : (height + sr.clutter)));
    ...
    fprintf(fd, "set grid\n");
    fprintf(fd, "set yrange [%2.3f to %2.3f]\n",
            sr.metric ? miny * METERS_PER_FOOT : miny,
            sr.metric ? maxy * METERS_PER_FOOT : maxy);
    fprintf(fd, "set y2range [%2.3f to %2.3f]\n",
            sr.metric ? min2y * METERS_PER_FOOT : min2y,
            sr.metric ? max2y * METERS_PER_FOOT : max2y);
        ...
        if (sr.clutter > 0.0) {
            if (sr.metric)
                fprintf(fd,
                        ...
                        "with lines, \"clutter.gp\" title \"Ground sr.clutter "
                        ...
                        sr.clutter * METERS_PER_FOOT, lrp.frq_mhz,
        ...
        if (sr.clutter > 0.0) {
            if (sr.metric)
                fprintf(
                    fd,
                    "plot \"profile.gp\" title \"Point-to-Point Profile\" with "
                    "lines, \"clutter.gp\" title \"Ground sr.clutter (%.2f "
                    "meters)\" with lines, \"reference.gp\" title \"Line Of "
